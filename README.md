...
- Голосовой ввод в Chrome: распознанный текст попадает в черновик сообщения и
  может быть отредактирован до отправки.
...
- Обезличенные метрики в Grafana Cloud при наличии токена в конфиге:
  [настройка и готовый JSON дашборда](docs/grafana/README.md).

## React UI и запуск одной командой

Фронтенд полностью перенесён на React + TypeScript. Бэкенд остаётся нативным
FastAPI-приложением с прежними Codex/OpenCode-интеграциями и SQLite.
Готовая React-сборка включена в репозиторий и Python-пакет: для её запуска
не нужны npm, Node.js или Docker. Node.js по-прежнему может требоваться самому
OpenCode; для разработки UI нужен Node 22.12+.

После установки `uv` запускайте из корня репозитория:

| Windows PowerShell | macOS Terminal |
| --- | --- |
| `./scripts/start-agent-web.ps1` | `bash scripts/start-agent-web.sh` |

Обе команды подготавливают Python-окружение через `uv` и запускают API и UI
на **http://127.0.0.1:8765** одним процессом. Ctrl+C останавливает приложение.
Обе используют конфигурацию и БД в `data/`, как скрипты перезапуска.
Перед первым использованием один раз разрешите корень проектов:
`uv run agent-web --data-dir ./data init --root /absolute/path/to/projects`
(Windows-пример: `--root F:\dev`).

Для LAN: `./scripts/start-agent-web.ps1 -AllowLan` или
`bash scripts/start-agent-web.sh --allow-lan`. Скрипты `restart-agent-web.*`
по-прежнему запускают сервер в фоне. Альтернатива для обеих ОС:
`uv run agent-web --data-dir ./data serve`.

У каждого чата есть URL `/projects/<project-id>/chats/<chat-id>`: обновление
страницы и кнопки назад/вперёд сохраняют контекст. Проекты и чаты доступны в
выдвижном боковом меню. Состояние агента и ответы обновляются через WebSocket;
при разрыве связи выполняются переподключение и сверка сохранённой истории.

Разработка, сборка и три браузерных сценария описаны в
...
8. Нажмите кнопку микрофона рядом с **Send**, разрешите браузеру доступ к
   микрофону и продиктуйте инструкцию. Нажмите её ещё раз, чтобы остановить
   запись, при необходимости отредактируйте текст и отправьте сообщение. Аудио
   не сохраняется Agent Web: агенту передаётся только получившийся текст.
