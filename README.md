...
Перед применением обновления у друга стоит запустить `uv run agent-web
run-tests`. Если тесты не проходят, не применяйте обновление: рабочая версия и
папка проекта останутся нетронутыми.
