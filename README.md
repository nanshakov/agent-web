...
- Экспорт открытого чата в Markdown; изображения и другие бинарные вложения
  автоматически добавляются в ZIP, а текстовые файлы раскрываются в переписке.
...
### Установите CLI LM Studio

Один раз откройте LM Studio и включите CLI (`lms`). Agent Web сам запускает
локальный сервер и загружает выбранную модель при создании OpenCode-чата или
первом сообщении после переключения.
...
запускается автоматически; вручную `opencode acp`, `lms server start` и
`lms load` запускать не нужно. Список моделей, длина контекста и лимит ответа
читаются из текущей конфигурации OpenCode.
...
внешнего переноса нажмите **Export** в заголовке открытого чата.

### Почему не видна модель LM Studio?

Проверьте, что `opencode models lm-studio` показывает модель, а её ключ в
`opencode.json` совпадает с ключом модели из `lms ls --llm`. Затем обновите
страницу Agent Web.
